...
server <- function(input, output, session) {
  ...
  observeEvent(input$TestDataButton, {
    ...
    test <- read_csv("testdata/RFB_diagnostic.csv")
  ...
  output$ForageStatus = renderText({ 
    ...
    if(is.null(RAW$data)==F & is.null(RAW$data$CPdist)==T)return(paste0("**No Central Place Distance, Can't Calculate Trips**"))
    
    ## if there are data but trips have not been calcualted then display this message (Need to have CPdist)
    if(is.null(RAW$data)==F & is.null(RAW$data$CPdist)==F & is.null(trips$data)==T)return(paste0("Data Chosen - Calcualte Trips with Green Button"))
    
    ## make message disappear
    if(is.null(RAW$data)==F & is.null(RAW$data$CPdist)==F & is.null(trips$data)==F)return(paste0(""))
  ...
  observeEvent(input$TripStatButton, {
    ...
    if(is.null(RAW$data)==F & is.null(RAW$data$CPdist)==T)return()
  ...
  output$TripClassPlot = renderPlot({
    ...
    if(input$PopSelect2 == "Individual"){
      ...
      ## make the plot
      trip_plot <- ggplot()+
        geom_point(data = filter(df_alltrips, Fill == "Discarded"), aes(x = DateTime, y = (CPdist/1000), col = Fill), alpha = 0.8, size = 1) +
    ...
    if(input$PopSelect2 == "Population"){
      ...
      ## make the plot
      trip_plot <- ggplot()+
        geom_point(data = filter(df_alltrips, Fill == "Discarded"), aes(x = SinceStart, y = (CPdist/1000), col = Fill), alpha = 0.8, size = 1) +
        scale_colour_manual(values = c("#C5C5C5"), name = "Trip duration\ntoo short")+
        new_scale_color()+
  ...
  observeEvent(input$SubSampButton, {
    ...
    if(input$SubSampInterval > 60){
      ...
      df_subsampled <- df_diagnostic %>%
        group_by(ID) %>%
        mutate(subsample = round_date(DateTime, unit = period(num = ((input$SubSampInterval)/60), units = "hours")), #round to nearest value of subsampling unit
  ...
  output$MeanSampInt <- renderValueBox({
    ...
    MeanInt <- round(mean((RAW$data$difftime/60), na.rm=T), digits = 1)
  ...
  output$SDSampInt <- renderValueBox({
    ...
    sdInt <- round(sd((RAW$data$difftime/60), na.rm=T), digits = 1)
  ...
  output$RangeSampInt <- renderValueBox({
    ...
    minInt <- round(min((RAW$data$difftime/60), na.rm=T) ,digits = 1)
    maxInt <- round(max((RAW$data$difftime/60), na.rm=T) ,digits = 1)
